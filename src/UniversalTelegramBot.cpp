    ...
    if (!error) {
      ...
      if (doc.containsKey("result")) {
        ...
        if (resultArrayLength > 0) {
          int newUpdateIndex = 0;
          // Step through all results
          for (int i = 0; i < resultArrayLength; i++) {
            JsonObject result = doc["result"][i];
            if (processResult(result, newUpdateIndex)) newUpdateIndex++;
          }
          doc.clear();
          ...
          #ifdef TELEGRAM_DEBUG  
            Serial.println(F("no new updates"));
          #endif
        }
      } else {
        #ifdef TELEGRAM_DEBUG  
      ...
      if (response.length() < 2) { // Too short a message. Maybe a connection issue
        #ifdef TELEGRAM_DEBUG  
            Serial.println(F("Parsing error: Update too short"));
        #endif
      } else {
...
bool UniversalTelegramBot::processResult(JsonObject result, int updateIndex) {
  int update_id = result["update_id"];
  // Check have we already dealt with this message (this shouldn't happen!)
  if (last_update_received != update_id) {
    last_update_received = update_id;
    TelegramUpdate *update = &updates[updateIndex];
    update->~TelegramUpdate(); // emptify previously created entities
    update->update_id = update_id;

    if (result.containsKey("message")) {
      TelegramMessage *message = new TelegramMessage();
      update->message = message->fromJsonObject(result["message"]);
    ...
    //   messages[messageIndex].message_id = message["message_id"].as<int>();  // added message id
      ...
      TelegramCallbackQuery *callbackQuery = new TelegramCallbackQuery();
      update->callback_query = callbackQuery->fromJsonObject(result["callback_query"]);
...
bool UniversalTelegramBot::banChatMember(const String& chat_id, const String& user_id, int until_date, bool revoke_messages) {
  ...
  if (revoke_messages != 0) {
    payload["revoke_messages"] = revoke_messages;
  }

  #ifdef TELEGRAM_DEBUG  
    Serial.println(F("banChatMember: BAN Chat Member"));
  #endif

  String response = sendPostToTelegram(BOT_CMD("banChatMember"), payload.as<JsonObject>());
  #ifdef _debug  
     Serial.print(F("banChatMember response:"));
...
bool UniversalTelegramBot::unbanChatMember(const String& chat_id, const String& user_id, bool only_if_banned) {
  ...
  #ifdef TELEGRAM_DEBUG  
    Serial.println(F("unbanChatMember: UNban Chat Member"));
  #endif

  String response = sendPostToTelegram(BOT_CMD("unbanChatMember"), payload.as<JsonObject>());
  #ifdef _debug  
     Serial.print(F("unbanChatMember response:"));
     Serial.println(response);
  #endif
  bool answer = checkForOkResponse(response);
  closeClient();
...
bool UniversalTelegramBot::deleteMessage(const String& chat_id, int message_id) {
  ...
  bool sent = false;
  #ifdef TELEGRAM_DEBUG 
    Serial.print(F("deleteMessage: SEND Delete Message: "));
...
bool UniversalTelegramBot::sendMessage(const String& chat_id, const String& text,
  ...
  if (reply_to_message_id != 0) {
    ...
    payload["allow_sending_without_reply"] = 1;
  }
